(function(){
 ...
 G.applyTheme=theme=>{if(!['companion','game','simple'].includes(theme))return;G.state.user.style=theme;document.documentElement.dataset.theme=theme;document.querySelectorAll('[data-theme-set]').forEach(b=>b.classList.toggle('active',b.dataset.themeSet===theme));G.save()};
 G.refresh=()=>{if(G.chart){G.chart.destroy();G.chart=null}G.cleanup.forEach(fn=>fn());G.cleanup=[];G.pages[page]?.();G.updateDemoProgress()};
 G.mount=(html,nav=true)=>{
  document.getElementById('app').innerHTML=html;
  document.getElementById('app').className=`page ${nav?'has-nav':''}`;
  const theme=G.state.user.style;const tabs=[['home',theme==='simple'?'chart-line':theme==='game'?'flag-checkered':'house',theme==='simple'?'總覽':theme==='game'?'營地':'首頁'],['gardens','plant','花圃'],['tasks','check-square',theme==='simple'?'待處理':theme==='game'?'任務':'照護'],['select','magnifying-glass','選株'],['settings','user-circle',theme==='simple'?'設定':'我的']];
  ...
  if(page==='home'&&nav&&G.state.user.style==='companion'){const a=document.createElement('a');a.className='fab';a.href=G.href('add-device.html');a.setAttribute('aria-label','新增設備');a.innerHTML=G.icon('plus');document.querySelector('.phone').append(a)}
 };
 ...
 G.confetti=()=>{if(G.state.user.style==='simple')return;if(matchMedia('(prefers-reduced-motion: reduce)').matches)return;const r=document.querySelector('.phone').getBoundingClientRect();window.confetti?.({particleCount:75,spread:65,startVelocity:28,origin:{x:(r.left+r.width/2)/innerWidth,y:.58},colors:['#82a661','#dfad7b','#b7cca0','#f2d283'],disableForReducedMotion:true,zIndex:90})};
 ...
 G.finish=async(id)=>{const before=ACCESSORIES.filter(G.unlocked).map(a=>a.id);const gained=G.completeTask(id);const fresh=ACCESSORIES.find(a=>G.unlocked(a)&&!before.includes(a.id));G.refresh();G.confetti();G.toast((G.state.user.style==='game'?`任務完成！+${gained} XP`:G.state.user.style==='simple'?'已記錄處理結果。':'照顧收到了，謝謝你。')+(fresh?` 已解鎖「${fresh.name}」！`:''))};
 ...
 G.updateDemoProgress=()=>{
  const summary=document.getElementById('demo-progress-summary');
  if(!summary)return;
  const next=ACCESSORIES.filter(a=>a.streak&&!G.unlocked(a)).sort((a,b)=>a.streak-b.streak)[0];
  summary.innerHTML=`<p>連續照護 <strong>${G.state.user.streak}</strong> 天 · 陪伴 <strong>${G.state.user.daysTogether}</strong> 天</p><p>已解鎖 <strong>${ACCESSORIES.filter(G.unlocked).length} / ${ACCESSORIES.length}</strong> 件造型</p>`;
  document.getElementById('demo-next-unlock').textContent=next?`再 ${next.streak-G.state.user.streak} 天，解鎖「${next.name}」。`:'所有天數條件的造型都已解鎖。';
 };
 G.advanceDemoDays=value=>{
  const days=Number(value);
  if(!G.demo||!Number.isInteger(days)||days<1||days>3650)return false;
  const before=new Set(ACCESSORIES.filter(G.unlocked).map(a=>a.id));
  const together=Number(G.state.user.daysTogether)||0;
  G.state.user.streak=(Number(G.state.user.streak)||0)+days;
  G.state.user.daysTogether=together+days;
  G.state.plants.forEach(p=>{p.days=(Number(p.days)||together)+days});
  // Treat this as simulated care completed today so the next task keeps the streak.
  G.state.lastCareDate=new Date().toLocaleDateString('en-CA');
  G.save();
  G.refresh();
  const fresh=ACCESSORIES.filter(a=>G.unlocked(a)&&!before.has(a.id));
  const message=`已增加 ${days} 天。${fresh.length?`已解鎖：${fresh.map(a=>a.name).join('、')}。`:`目前連續照護 ${G.state.user.streak} 天。`}`;
  const status=document.getElementById('demo-progress-status');
  if(status)status.textContent=message;else G.toast(message);
  if(fresh.length)G.confetti();
  return true;
 };
 G.bindDemoProgress=el=>{
  G.updateDemoProgress();
  el.querySelectorAll('[data-demo-days]').forEach(b=>b.onclick=()=>G.advanceDemoDays(b.dataset.demoDays));
  el.querySelector('#demo-days-form').onsubmit=e=>{
   e.preventDefault();
   const input=el.querySelector('#demo-days');
   if(input.reportValidity())G.advanceDemoDays(input.value);
  };
 };
